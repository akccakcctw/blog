...
Free 方案每月 **5,000 次唯一轉換**免費。「唯一」指「來源圖 + 參數組合」的組合，
同一組只算一次，之後走快取不重複計次。現有 10 張圖就只佔 10 次，要撞到上限得在
一個月內上傳約 5,000 張新圖。
...
| 已快取的轉換 | **繼續正常服務** |
| 新的轉換 | 回 **9422 錯誤** |
| 費用 | Free 方案**不會被收費**，是硬性上限 |
| 重置 | 每月計費週期歸零 |

因為 plugin 是全站套用，超量會讓新圖直接破圖。所以參數裡帶了
**`onerror=redirect`** 作為失效保護：轉換失敗時改為 307 轉址到原圖，讀者看到的是
未最佳化的圖片而不是破圖。這個機制要求原圖與轉換服務位於同一網域，正好符合本站
架構（兩者都在 `static.rex-tsou.com`）。

實測：正常情況輸出完全不變；來源不存在時由 `404` 變成 `307 → 原圖網址`。

## 改了 plugin 卻沒生效？

Astro 會把**已渲染的 Markdown HTML** 快取在 `node_modules/.astro`。Markdown 本身
沒變動時它不會重跑 rehype plugin，所以改了 `src/plugins/rehype-r2-image.mjs` 之後，
產出可能仍是舊的 —— **而且建置會成功、沒有任何錯誤訊息**。Netlify 會快取整個
`node_modules`，這個陳舊狀態還會跨建置保留。

`package.json` 的 build script 已固定帶 `--force` 來清除該快取：

```json
"build": "astro check && astro build --force"
```

**請勿拿掉這個旗標。** 實測代價是每次建置多約 2.4 秒（12.9s → 15.3s），佔整個
Netlify 部署時間約 3%，遠低於這個無聲失效的排查成本。

`pnpm dev` 不受影響，開發時仍走快取。
